...
import { FounderNote } from "@/registry/default/founder-note"
...
function ComponentPreview({
  ...
  if (slug === "blue-cta-button") {
    return (
      <div className="preview-grid relative flex min-h-72 items-center justify-center overflow-hidden rounded-2xl border">
        <BlueCtaButton tone={blueCtaTone} platform={blueCtaPlatform}>
  ...
  if (slug === "founder-note") {
    return (
      <PreviewSurface>
        <FounderNote
          {...founderNoteExample}
          links={founderNoteExample.links.map((link) => ({
            ...link,
            icon: <FounderLinkIcon label={link.label} />,
          }))}
...
function FounderLinkIcon({ label }: { label: string }) {
